...
contract('User', function(accounts) {
    ...
    describe("Error Cases", async() => {
        ...
        it("Get Non-existent User", async() =>  {
           ...
           await tryCatch(myUserInstance.get(nonOwner), errTypes.revert);
        });
        it("Authentication for Non-existent User", async() =>  {
  
            await tryCatch(myUserInstance.authenticate({from: nonOwner}), errTypes.revert);
        });
        it("Delete Non-existent User", async() =>  {
  
            await tryCatch(myUserInstance.destroy({from: nonOwner}), errTypes.revert);
        });
    });
});
